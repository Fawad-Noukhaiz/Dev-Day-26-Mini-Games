...
const HIGH_SCORE_STORAGE_KEY = 'pinballHighScore_v2';
const SCORE_API_URL = `${API_BASE_URL}/scores`;
const PINBALL_API_KEY = 'mgk_e7cc76c809d96155221ca777b6cd4645a58e8ba4efb274182d3939a0b3fb416d';
const PINBALL_GAME_ID = 'd5a25117-def7-4840-a428-e3a458210b78';
...
function PinballGame() {
  ...
  const [highScore, setHighScore] = useState(
    parseInt(localStorage.getItem(HIGH_SCORE_STORAGE_KEY) || '0')
  );
  ...
  useEffect(() => {
    playerCodeRef.current = normalizeCode(playerCode);
  }, [playerCode]);

  const lookupParticipant = useCallback(async (rawCode) => {
    const code = normalizeCode(rawCode);
    if (!code) throw new Error('Enter your participant code to launch.');
  ...
  }, [beginGameSession, lookupParticipant, playerCode]);
    ...
    try {
      const response = await fetch(SCORE_API_URL, {
        ...
        headers: {
          ...
          'x-api-key': PINBALL_API_KEY,
        },
        body: JSON.stringify({
          userCode,
          gameId: PINBALL_GAME_ID,
          score: finalScore,
  ...
  const gameLoop = useCallback(() => {
    ...
    if (b.y > DRAIN_Y && b.x < LAUNCH_LANE_LEFT && g.drainCooldown <= 0) {
      ...
      if (g.ballsLeft <= 0) {
        ...
        g.ball = null;
        submitScore(finalScore, playTime);
    ...
    setTick((t) => t + 1);
  }, [gameState, g, addScore, addParticles, initBall, highScore, submitScore]);
          ...
          <div className="start-content">
            ...
            <div className="player-entry">
              <input
                ...
                maxLength={24}
              />
              {playerName && <div className="player-name-badge">Player: {playerName}</div>}
              {apiMessage && <div className="player-api-message">{apiMessage}</div>}
            </div>
            <button className="start-button" onClick={startGame} disabled={isStarting}>
              {isStarting ? 'VERIFYING' : 'START'}
            </button>
          </div>
        </div>
      )}
      ...
      {gameState === 'gameOver' && (
        <div className="overlay-screen game-over-screen">
          ...
          <div className="game-over-content">
            ...
            {playerName && <div className="player-name-badge game-over-player">Player: {playerName}</div>}
            {apiMessage && <div className="player-api-message">{apiMessage}</div>}

            <div className="game-over-buttons">
              <button
                className="play-again-button"
                onClick={beginGameSession}
              >
                PLAY AGAIN
              </button>
