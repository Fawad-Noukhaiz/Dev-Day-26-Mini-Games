  ...
  const gameLoop = useCallback(() => {
    ...
    for (let step = 0; step < SUB; step++) {
      ...
      for (const bumper of BUMPERS) {
        const res = ballCircleCollision(b, bumper.x, bumper.y, bumper.radius, 6);
        if (res && res.hit) {
          b.x = res.x;
          b.y = res.y;
          b.vx = res.vx;
      ...
      for (const ro of ROLLOVERS) {
        if (g.rolloverStates[ro.id] && Date.now() - g.rolloverStates[ro.id] < 500) continue;
        const res = ballRectCollision(b, ro.x, ro.y, ro.width, ro.height);
        if (res && res.hit) {
          g.rolloverStates[ro.id] = Date.now();
    ...
    if (Date.now() - g.lastBumperHit > 5000 && g.multiplier > 1) g.multiplier = 1;
        ...
        <div className="pinball-board-wrapper" style={{ transform: `scale(${scale})` }}>
          ...
          >
            ...
            {[180, 280, 380, 480, 580].map((y, i) => (
              <g key={`left-light-${i}`}>
                <circle cx={12} cy={y} r={5} fill={['#ff3366', '#00aaff', '#ffaa00', '#00ffaa', '#ff66aa'][i]} opacity={0.5} />
                <circle cx={12} cy={y} r={2} fill="#fff" opacity={0.8} />
              </g>
            ))}
            {[200, 300, 400, 500, 600].map((y, i) => (
              <g key={`right-light-${i}`}>
                <circle cx={BOARD_WIDTH - 12} cy={y} r={5} fill={['#00aaff', '#ff3366', '#00ffaa', '#ffaa00', '#aa66ff'][i]} opacity={0.5} />
            ...
            {/* Arrow indicators in launch lane */}
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <path
                key={`arr${i}`}
            ...
            {SLINGSHOTS.map((sling) => {
              const isHit = g.slingshotHits[sling.id] && Date.now() - g.slingshotHits[sling.id] < 150;
              return (
                <g key={sling.id}>
            ...
            {/* Rollovers */}
            {ROLLOVERS.map((ro) => {
              const isActive = g.rolloverStates[ro.id] && Date.now() - g.rolloverStates[ro.id] < 500;
              return (
                <g key={ro.id}>
                  <rect
                    x={ro.x}
            ...
            {/* Bumpers */}
            {BUMPERS.map((bumper) => {
              const isHit = g.bumperHits[bumper.id] && Date.now() - g.bumperHits[bumper.id] < 150;
              const scale = isHit ? 1.12 : 1;
              return (
                <g key={bumper.id} transform={`translate(${bumper.x},${bumper.y}) scale(${scale})`}>
