...
const normalizeLookupCode = (value) => sanitizeCode(value).toUpperCase();
const getParticipantCode = (data, fallback = "") =>
  sanitizeCode(
    data?.userCode ||
    data?.playerCode ||
    data?.minigamePlayerCode ||
    data?.participantCode ||
    data?.code ||
    data?.player?.code ||
    data?.minigamePlayer?.code ||
    fallback
  );
const getParticipantName = (data, fallback = "") =>
  data?.fullName || data?.name || data?.player?.fullName || fallback;
...
export default function App() {
  ...
  const playerCodeRef = useRef(
    sanitizeCode(localStorage.getItem(PLAYER_CODE_STORAGE_KEY) || "")
  );

  useEffect(() => { sRef.current = score; }, [score]);
  useEffect(() => { vRef.current = view; }, [view]);
  useEffect(() => { playerCodeRef.current = sanitizeCode(playerCode); }, [playerCode]);

  const lookupParticipant = useCallback(async (rawCode) => {
    const lookupCode = normalizeLookupCode(rawCode);
    if (!lookupCode) throw new Error("Enter your participant code to play.");

    const data = await getParticipant(lookupCode);
    const code = getParticipantCode(data, lookupCode);
    return { code, fullName: getParticipantName(data, code) };
  }, []);
